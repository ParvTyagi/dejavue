...
const pin = (color: string) =>
  L.divIcon({
    ...
    html: `<span style="position:relative;display:block;width:22px;height:22px">
      ...
      <span style="position:absolute;inset:5px;border-radius:9999px;background:${color};box-shadow:0 0 0 2px #08080a,0 0 16px ${color}"></span>
    </span>`,
  });

/** Claimed place and resolved scene on dark tiles, joined by a flowing dashed line. */
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-line">
      ...
      >
        ...
        />
        {claim && scene && (
          <Polyline positions={bounds} pathOptions={{ color: '#d4f75c', weight: 2, dashArray: '8 8', className: 'dash-flow', opacity: 0.8 }} />
        )}
        {claim && (
          <Marker position={[claim.lat, claim.lng]} icon={pin('#6cb4ff')}>
        ...
        {scene && (
          <Marker position={[scene.lat, scene.lng]} icon={pin('#ff6b4f')}>
      ...
      {distanceKm !== undefined && (
        <div className="pointer-events-none absolute top-3 right-3 z-[500] rounded-lg border border-line-strong bg-bg/85 px-3 py-1.5 font-mono text-xs text-accent backdrop-blur">
