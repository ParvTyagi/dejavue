...
import { ScrambleText, useVisibleInterval } from '@/components/ui/motion';
...
export function HeroPreview() {
  ...
  useVisibleInterval(ref, () => setStep((s) => (s + 1) % 5), 1400);

  const stamped = step >= 3;

  return (
    <div ref={ref} style={{ '--rise-delay': '0.25s' } as React.CSSProperties} className="rise relative mx-auto w-full max-w-md lg:max-w-none">
      <div className="rounded-2xl border border-line-strong bg-bg/90 shadow-xl shadow-black/[0.04]">
        ...
        <div className="grid grid-cols-[104px_1fr] gap-4 p-4 sm:grid-cols-[150px_1fr] sm:p-5">
          <div className="relative aspect-[4/5] self-start overflow-hidden rounded-xl border border-line bg-[linear-gradient(160deg,#e5e5e5,#f5f5f5_55%,#e0e0e0)]">
            <svg viewBox="0 0 100 125" className="absolute inset-0 size-full opacity-70" aria-hidden>
              <path d="M0 80 Q20 70 35 78 T70 74 T100 80 V125 H0Z" fill="#a3a3a3" opacity="0.6" />
              <path d="M0 92 Q25 84 50 92 T100 90 V125 H0Z" fill="#737373" opacity="0.35" />
              <rect x="18" y="48" width="14" height="26" fill="#525252" />
              <rect x="38" y="40" width="18" height="34" fill="#404040" />
              <rect x="62" y="52" width="12" height="22" fill="#525252" />
            </svg>
            <div className="scan-band animate-scan" />
            <div className="absolute inset-x-0 bottom-0 bg-white/80 px-2 py-1.5 font-mono text-[10px] text-ink">
          ...
          <AnimatePresence mode="wait">
            {stamped ? (
              <motion.div
                ...
                className="rounded-lg border-2 border-bad px-4 py-1.5 font-mono text-sm font-bold tracking-[0.2em] text-bad uppercase"
              >
                Old media · new claim
              </motion.div>
            ) : (
