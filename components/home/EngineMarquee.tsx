import { ENGINE_COLOR, ENGINE_ICON } from '@/components/audit/icons';
...
const ENGINES = SEARCHED.map((id) => ({ name: ENGINE_LABEL[id], icon: ENGINE_ICON[id], color: ENGINE_COLOR[id] }));
    ...
    <div className="relative mx-auto max-w-6xl overflow-hidden py-6 [mask-image:linear-gradient(to_right,transparent,black_15%,black_85%,transparent)]">
      <ul className="flex w-max animate-marquee gap-10 hover:[animation-play-state:paused]">
        {row.map(({ name, icon: Icon, color }, i) => (
          <li key={i} aria-hidden={i >= ENGINES.length} className="flex items-center gap-2 text-sm whitespace-nowrap text-muted">
